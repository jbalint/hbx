...
module Database.Hbx.Proto.Datatypes.Scalar.Type (Type(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Type where
  ...
  fromEnum V_STRING = 8
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Database.Hbx.Proto.Datatypes.Scalar.Type") .
      toMaybe'Enum
  succ V_SINT = V_UINT
  succ V_UINT = V_NULL
  ...
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Database.Hbx.Proto.Datatypes.Scalar.Type"
  pred V_UINT = V_SINT
  pred V_NULL = V_UINT
  ...
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Database.Hbx.Proto.Datatypes.Scalar.Type"
...
instance P'.ReflectEnum Type where
  ...
  reflectEnumInfo _
   = P'.EnumInfo
      (P'.makePNF (P'.pack ".Mysqlx.Datatypes.Scalar.Type") [] ["Database", "Hbx", "Proto", "Datatypes", "Scalar"] "Type")
      ["Database", "Hbx", "Proto", "Datatypes", "Scalar", "Type.hs"]
