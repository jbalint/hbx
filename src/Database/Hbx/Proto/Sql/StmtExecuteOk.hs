...
module Database.Hbx.Proto.Sql.StmtExecuteOk (StmtExecuteOk(..)) where
import Prelude ((+), (/))
...
instance P'.ReflectDescriptor StmtExecuteOk where
  ...
  reflectDescriptorInfo _
   = Prelude'.read
      "DescriptorInfo {descName = ProtoName {protobufName = FIName \".Mysqlx.Sql.StmtExecuteOk\", haskellPrefix = [], parentModule = [MName \"Database\",MName \"Hbx\",MName \"Proto\",MName \"Sql\"], baseName = MName \"StmtExecuteOk\"}, descFilePath = [\"Database\",\"Hbx\",\"Proto\",\"Sql\",\"StmtExecuteOk.hs\"], isGroup = False, fields = fromList [], keys = fromList [], extRanges = [], knownKeys = fromList [], storeUnknown = False, lazyFields = False}"
