...
module Database.Hbx.Proto.ClientMessages.Type (Type(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Type where
  ...
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Database.Hbx.Proto.ClientMessages.Type") .
      toMaybe'Enum
  ...
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Database.Hbx.Proto.ClientMessages.Type"
  ...
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Database.Hbx.Proto.ClientMessages.Type"
...
instance P'.ReflectEnum Type where
  reflectEnum
   = [(1, "CON_CAPABILITIES_GET", CON_CAPABILITIES_GET), (2, "CON_CAPABILITIES_SET", CON_CAPABILITIES_SET),
      ...
      (25, "EXPECT_CLOSE", EXPECT_CLOSE)]
  reflectEnumInfo _
   = P'.EnumInfo (P'.makePNF (P'.pack ".Mysqlx.ClientMessages.Type") [] ["Database", "Hbx", "Proto", "ClientMessages"] "Type")
      ["Database", "Hbx", "Proto", "ClientMessages", "Type.hs"]
