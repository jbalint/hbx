...
module Database.Hbx.Proto.Expect.Open.Condition.ConditionOperation (ConditionOperation(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum ConditionOperation where
  ...
  toEnum
   = P'.fromMaybe
      (Prelude'.error "hprotoc generated code: toEnum failure for type Database.Hbx.Proto.Expect.Open.Condition.ConditionOperation")
      . toMaybe'Enum
  succ EXPECT_OP_SET = EXPECT_OP_UNSET
  succ _
   = Prelude'.error "hprotoc generated code: succ failure for type Database.Hbx.Proto.Expect.Open.Condition.ConditionOperation"
  pred EXPECT_OP_UNSET = EXPECT_OP_SET
  pred _
   = Prelude'.error "hprotoc generated code: pred failure for type Database.Hbx.Proto.Expect.Open.Condition.ConditionOperation"
  ...
  reflectEnumInfo _
   = P'.EnumInfo
      (P'.makePNF (P'.pack ".Mysqlx.Expect.Open.Condition.ConditionOperation") []
        ["Database", "Hbx", "Proto", "Expect", "Open", "Condition"]
        "ConditionOperation")
      ["Database", "Hbx", "Proto", "Expect", "Open", "Condition", "ConditionOperation.hs"]
