...
module Database.Hbx.Proto.Expect.Open.CtxOperation (CtxOperation(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum CtxOperation where
  ...
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Database.Hbx.Proto.Expect.Open.CtxOperation") .
      toMaybe'Enum
  succ EXPECT_CTX_COPY_PREV = EXPECT_CTX_EMPTY
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Database.Hbx.Proto.Expect.Open.CtxOperation"
  pred EXPECT_CTX_EMPTY = EXPECT_CTX_COPY_PREV
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Database.Hbx.Proto.Expect.Open.CtxOperation"
...
instance P'.ReflectEnum CtxOperation where
  ...
  reflectEnumInfo _
   = P'.EnumInfo
      (P'.makePNF (P'.pack ".Mysqlx.Expect.Open.CtxOperation") [] ["Database", "Hbx", "Proto", "Expect", "Open"] "CtxOperation")
      ["Database", "Hbx", "Proto", "Expect", "Open", "CtxOperation.hs"]
