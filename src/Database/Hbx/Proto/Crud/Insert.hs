...
module Database.Hbx.Proto.Crud.Insert (Insert(..)) where
import Prelude ((+), (/))
...
import qualified Database.Hbx.Proto.Crud.Column as Database.Hbx.Proto.Crud (Column)
import qualified Database.Hbx.Proto.Crud.DataModel as Database.Hbx.Proto.Crud (DataModel)
import qualified Database.Hbx.Proto.Crud.Insert.TypedRow as Database.Hbx.Proto.Crud.Insert (TypedRow)
 
data Insert = Insert{collection :: !(Database.Hbx.Proto.Crud.Collection),
                     data_model :: !(P'.Maybe Database.Hbx.Proto.Crud.DataModel),
                     projection :: !(P'.Seq Database.Hbx.Proto.Crud.Column),
                     row :: !(P'.Seq Database.Hbx.Proto.Crud.Insert.TypedRow)}
            deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data)
 
instance P'.Mergeable Insert where
  mergeAppend (Insert x'1 x'2 x'3 x'4) (Insert y'1 y'2 y'3 y'4)
...
instance P'.Default Insert where
  defaultValue = Insert P'.defaultValue P'.defaultValue P'.defaultValue P'.defaultValue
 
instance P'.Wire Insert where
  wireSize ft' self'@(Insert x'1 x'2 x'3 x'4)
   = case ft' of
       10 -> calc'Size
       11 -> P'.prependMessageSize calc'Size
       _ -> P'.wireSizeErr ft' self'
    where
        calc'Size = (P'.wireSizeReq 1 11 x'1 + P'.wireSizeOpt 1 14 x'2 + P'.wireSizeRep 1 11 x'3 + P'.wireSizeRep 1 11 x'4)
  wirePut ft' self'@(Insert x'1 x'2 x'3 x'4)
   = case ft' of
       10 -> put'Fields
       11 -> do
...
instance P'.TextMsg Insert where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'collection, parse'data_model, parse'projection, parse'row]) P'.spaces
