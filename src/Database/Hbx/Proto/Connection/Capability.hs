...
module Database.Hbx.Proto.Connection.Capability (Capability(..)) where
import Prelude ((+), (/))
import qualified Prelude as Prelude'
import qualified Data.Typeable as Prelude'
import qualified Data.Data as Prelude'
import qualified Text.ProtocolBuffers.Header as P'
import qualified Database.Hbx.Proto.Datatypes.Any as Database.Hbx.Proto.Datatypes (Any)
 
data Capability = Capability{name :: !(P'.Utf8), value :: !(Database.Hbx.Proto.Datatypes.Any)}
                deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data)
 
instance P'.Mergeable Capability where
  mergeAppend (Capability x'1 x'2) (Capability y'1 y'2) = Capability (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2)
 
instance P'.Default Capability where
  defaultValue = Capability P'.defaultValue P'.defaultValue
 
instance P'.Wire Capability where
  wireSize ft' self'@(Capability x'1 x'2)
   = case ft' of
       10 -> calc'Size
  ...
  wirePut ft' self'@(Capability x'1 x'2)
   = case ft' of
       10 -> put'Fields
       11 -> do
