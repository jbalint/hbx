...
module Database.Hbx.Proto.Expr.Identifier (Identifier(..)) where
import Prelude ((+), (/))
...
data Identifier = Identifier{name :: !(P'.Utf8), schema_name :: !(P'.Maybe P'.Utf8)}
                deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data)
 
instance P'.Mergeable Identifier where
  mergeAppend (Identifier x'1 x'2) (Identifier y'1 y'2) = Identifier (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2)
...
instance P'.Wire Identifier where
  ...
  wirePut ft' self'@(Identifier x'1 x'2)
   = case ft' of
       10 -> put'Fields
       11 -> do
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{schema_name = Prelude'.Just new'Field}) (P'.wireGet 9)
...
instance P'.TextMsg Identifier where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'name, parse'schema_name]) P'.spaces
