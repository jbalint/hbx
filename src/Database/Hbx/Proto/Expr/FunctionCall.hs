...
module Database.Hbx.Proto.Expr.FunctionCall (FunctionCall(..)) where
import Prelude ((+), (/))
...
import qualified Database.Hbx.Proto.Expr.Identifier as Database.Hbx.Proto.Expr (Identifier)
 
data FunctionCall = FunctionCall{name :: !(Database.Hbx.Proto.Expr.Identifier), param :: !(P'.Seq Database.Hbx.Proto.Expr.Expr)}
