...
module Database.Hbx.Proto.Expr.Expr.Type (Type(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Type where
  ...
  fromEnum ARRAY = 8
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Database.Hbx.Proto.Expr.Expr.Type") .
      toMaybe'Enum
  succ IDENT = LITERAL
  ...
  succ OBJECT = ARRAY
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Database.Hbx.Proto.Expr.Expr.Type"
  pred LITERAL = IDENT
  ...
  pred ARRAY = OBJECT
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Database.Hbx.Proto.Expr.Expr.Type"
...
instance P'.ReflectEnum Type where
  ...
  reflectEnumInfo _
   = P'.EnumInfo (P'.makePNF (P'.pack ".Mysqlx.Expr.Expr.Type") [] ["Database", "Hbx", "Proto", "Expr", "Expr"] "Type")
      ["Database", "Hbx", "Proto", "Expr", "Expr", "Type.hs"]
