...
module Database.Hbx.Proto.Expr.Operator (Operator(..)) where
import Prelude ((+), (/))
...
data Operator = Operator{name :: !(P'.Utf8), param :: !(P'.Seq Database.Hbx.Proto.Expr.Expr)}
              deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data)
 
instance P'.Mergeable Operator where
  mergeAppend (Operator x'1 x'2) (Operator y'1 y'2) = Operator (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2)
 
instance P'.Default Operator where
  defaultValue = Operator P'.defaultValue P'.defaultValue
 
instance P'.Wire Operator where
  wireSize ft' self'@(Operator x'1 x'2)
   = case ft' of
       10 -> calc'Size
  ...
  wirePut ft' self'@(Operator x'1 x'2)
   = case ft' of
       10 -> put'Fields
       11 -> do
