...
module Database.Hbx.Proto.Resultset.FetchDoneMoreOutParams (FetchDoneMoreOutParams(..)) where
import Prelude ((+), (/))
...
instance P'.ReflectDescriptor FetchDoneMoreOutParams where
  ...
  reflectDescriptorInfo _
   = Prelude'.read
      "DescriptorInfo {descName = ProtoName {protobufName = FIName \".Mysqlx.Resultset.FetchDoneMoreOutParams\", haskellPrefix = [], parentModule = [MName \"Database\",MName \"Hbx\",MName \"Proto\",MName \"Resultset\"], baseName = MName \"FetchDoneMoreOutParams\"}, descFilePath = [\"Database\",\"Hbx\",\"Proto\",\"Resultset\",\"FetchDoneMoreOutParams.hs\"], isGroup = False, fields = fromList [], keys = fromList [], extRanges = [], knownKeys = fromList [], storeUnknown = False, lazyFields = False}"
