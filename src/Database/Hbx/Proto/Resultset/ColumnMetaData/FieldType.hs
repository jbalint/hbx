...
module Database.Hbx.Proto.Resultset.ColumnMetaData.FieldType (FieldType(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum FieldType where
  ...
  toEnum
   = P'.fromMaybe
      (Prelude'.error "hprotoc generated code: toEnum failure for type Database.Hbx.Proto.Resultset.ColumnMetaData.FieldType")
      . toMaybe'Enum
  succ SINT = UINT
  succ UINT = DOUBLE
  succ DOUBLE = FLOAT
  ...
  succ ENUM = BIT
  succ BIT = DECIMAL
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Database.Hbx.Proto.Resultset.ColumnMetaData.FieldType"
  pred UINT = SINT
  pred DOUBLE = UINT
  pred FLOAT = DOUBLE
  pred BYTES = FLOAT
  ...
  pred BIT = ENUM
  pred DECIMAL = BIT
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Database.Hbx.Proto.Resultset.ColumnMetaData.FieldType"
  ...
  reflectEnumInfo _
   = P'.EnumInfo
      (P'.makePNF (P'.pack ".Mysqlx.Resultset.ColumnMetaData.FieldType") []
        ["Database", "Hbx", "Proto", "Resultset", "ColumnMetaData"]
        "FieldType")
      ["Database", "Hbx", "Proto", "Resultset", "ColumnMetaData", "FieldType.hs"]
